    ...
    do {                                                                                                   \
        ...
        if (0) {                                                                                           \
            std::cout << "<key>:" << str.substr(last, key - last)                                          \
                      << " <decoded>:" << LT::StringUtil::UrlDecode(str.substr(last, key - last))       \
                      << " <value>:" << str.substr(key + 1, pos - key - 1)                                 \
                      << " <decoded>:" << LT::StringUtil::UrlDecode(str.substr(key + 1, pos - key - 1)) \
                      << std::endl;                                                                        \
        }                                                                                                  \
                                                                                                           \
        m.insert(std::make_pair(LT::StringUtil::UrlDecode(trim(str.substr(last, key - last))),          \
                                LT::StringUtil::UrlDecode(str.substr(key + 1, pos - key - 1))));        \
        if (pos == std::string::npos) {                                                                    \
            break;                                                                                         \
        }                                                                                                  \
        ++pos;                                                                                             \
...
void HttpRequest::initCookies() {
    ...
    PARSE_PARAM(cookie, m_cookies, ';', LT::StringUtil::Trim);
    m_parserParamFlag |= 0x4;
}
...
void HttpResponse::setCookie(const std::string &key, const std::string &val,
    ...
    ss << key << "=" << val;
    if (expired > 0) {
        ss << ";expires=" << LT::Time2Str(expired, "%a, %d %b %Y %H:%M:%S") << " GMT";
    }
    if (!domain.empty()) {
        ss << ";domain=" << domain;
    }
