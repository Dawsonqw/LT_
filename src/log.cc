...
void LogEvent::vprintf(const char *fmt, va_list ap) {
    char *buf = nullptr;
    int len = vasprintf(&buf, fmt, ap);///使用参数列表发送格式化输出到字符串
    if(len != -1) {
...
void LogFormatter::init() {
    ...
    /**
     * 以下逻辑可以画图：
     * 自己设计的想法：解析的模板大概如下：[%m:%p]:{%d}
     * 也就是有%跟着的是需要进行转义的 对于其它的是普通字符 原样输出即可
     *
     * sylar设计的仅仅通过一个变量进行跳转 不太好理解
     * todo：重新设计一个解析方案
     * **/
    size_t i = 0;
...
std::string LogFormatter::format(LogEvent::ptr event) {
    std::stringstream ss;
    ///多态操作：会具体调用子类进行赋值操作：涉及设计模式：工厂模式：todo：深入学习工厂模式
    for(auto &i:m_items) {
        i->format(ss, event);
    }
    return ss.str();
...
LT::ConfigVar<std::set<LogDefine>>::ptr g_log_defines = 
    LT::Config::Lookup("logs", std::set<LogDefine>(), "logs config");

struct LogIniter {
    LogIniter() {
        g_log_defines->addListener([](const std::set<LogDefine> &old_value, const std::set<LogDefine> &new_value){
            LT_LOG_INFO(LT_LOG_ROOT()) << "on log config changed";
            for(auto &i : new_value) {
                auto it = old_value.find(i);
                LT::Logger::ptr logger;
                if(it == old_value.end()) {
                    // 新增logger
                    logger = LT_LOG_NAME(i.name);
                } else {
                    if(!(i == *it)) {
                        // 修改的logger
                        logger == LT_LOG_NAME(i.name);
                ...
                for(auto &a : i.appenders) {
                    LT::LogAppender::ptr ap;
                    if(a.type == 1) {
                        ap.reset(new FileLogAppender(a.file));
                    } else if(a.type == 2) {
                        // 如果以daemon方式运行，则不需要创建终端appender
                        if(!LT::EnvMgr::GetInstance()->has("d")) {
            ...
            for(auto &i : old_value) {
                ...
                if(it == new_value.end()) {
                    auto logger = LT_LOG_NAME(i.name);
