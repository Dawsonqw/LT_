#ifndef __LT_SCHEDULER_H__
#define __LT_SCHEDULER_H__
...
/**
 * 概览：scheduler实际上是一个线程池，通过一个task队列封装回调和协程，一个线程池，每个线程绑定run函数不断从队列拿任务执行，
 * 通过一个task对象拿到任务后取出对应的协程或者回调，再同一包装为协程，协程执行resume方法恢复回调函数的执行从而实现了协程的调度，
 * 强调：本质方法是线程池 同步方法是通过加锁实现的，这里也是改进点 todo
 * **/
