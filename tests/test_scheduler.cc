 ...
 */
void test_fiber1() {
    LT_LOG_INFO(g_logger) << "test_fiber1 begin";
    ...
    LT_LOG_INFO(g_logger) << "before test_fiber1 yield";
    LT::Fiber::GetThis()->yield();
    LT_LOG_INFO(g_logger) << "after test_fiber1 yield";

    LT_LOG_INFO(g_logger) << "test_fiber1 end";
}
 ...
 */
void test_fiber2() {
    LT_LOG_INFO(g_logger) << "test_fiber2 begin";
    ...
    LT_LOG_INFO(g_logger) << "test_fiber2 end";
}

void test_fiber3() {
    LT_LOG_INFO(g_logger) << "test_fiber3 begin";
    LT_LOG_INFO(g_logger) << "test_fiber3 end";
}

void test_fiber5() {
    static int count = 0;

    LT_LOG_INFO(g_logger) << "test_fiber5 begin, i = " << count;
    LT_LOG_INFO(g_logger) << "test_fiber5 end i = " << count;
 ...
 */
void test_fiber4() {
    LT_LOG_INFO(g_logger) << "test_fiber4 begin";
    
    for (int i = 0; i < 3; i++) {
        LT::Scheduler::GetThis()->schedule(test_fiber5, LT::GetThreadId());
    }

    LT_LOG_INFO(g_logger) << "test_fiber4 end";
}
